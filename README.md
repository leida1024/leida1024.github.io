...
站点源码。公开内容位于 `docs/`；在 VS Code 中编辑 Markdown，提交并推送到
`main` 后，GitHub Actions 会自动构建并发布到
<https://leida1024.github.io/>。

站点方向、迁移审核和来源清单等内部记录统一保存在 `management/`，不参与站点
构建。仓库的长期维护约定见 `CLAUDE.md`。
...
`slug`。模板会自动更新主页、排序、分类和时间线，不需要手工修改导航。本仓库只
保存源码，生成的 `site/` 目录不提交。页面时间使用带时区的 ISO 8601 值，当前文章
统一显示北京时间 `+08:00` 并保留到秒。

## 站点结构

- `主页` 是按创建时间排列的完整内容流，可按分类筛选和切换排序。
- `专题` 将现有分类按阅读方向组织为系统开发、硬件平台、软件效率和基础记录四组。
- `分类` 与 `时间线` 根据文章 frontmatter 自动生成；时间线按年、月组织文章。
- `关于` 说明站点内容范围和阅读入口，通过主页快捷链接访问。

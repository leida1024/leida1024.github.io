...
| `10-ubuntu-usages/03-server.md` | 配置教程 | [Ubuntu Server 局域网、SSH 与代理配置](../../articles/linux/ubuntu-server-lan-ssh-proxy/ubuntu-server-lan-ssh-proxy.md) | 删除关闭整机防火墙和允许 root 登录的建议，按网络、SSH、代理重排 |
| `01-configs-backup/` 与安装脚本 | 配置方法 | [Linux dotfiles 备份与恢复脚本的边界](../../articles/linux/dotfiles-backup-and-restore/dotfiles-backup-and-restore.md) | 不发布原始自动化，提炼可回滚的配置恢复方法 |
| `10-office/word.md` | 操作速记 | [Word 删除整页与空白页](../../articles/office/word-delete-page/word-delete-page.md) | 从单条快捷操作扩展为按分页原因排查 |
...
- `10-ubuntu-usages/nerdtree.md` 与离线 Vim 插件脚本合并进
  [Vim 基础配置与搜索替换](../../articles/linux/vim-notes/vim-notes.md)，只保留仍可确认的
  NERDTree 操作与原生 package 安装路径。
- 离线 Zsh、Oh My Zsh、Powerlevel10k 和 tmux 配置不再各建文章。安装边界已经分别由
  [Zsh 与 Oh My Zsh 安装边界](../../articles/linux/zsh-notes/zsh-notes.md)、
  [tmux 基础配置与自动进入会话](../../articles/linux/tmux-notes/tmux-notes.md) 和 dotfiles 文章覆盖。
- 三份 ZCU102 PDF 没有复制进站点。已有
  [ZCU102 时钟与板卡资料索引](../../articles/fpga/zcu102-reference-notes/zcu102-reference-notes.md) 指向官方资料。
